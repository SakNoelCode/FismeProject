                            ...
                            <tbody>
                                ...
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        @if ($acta->tipoacta_id == 1 && $acta->fecha_proveido)
                                        ...
                                        <!--Acta de revisióm-->
                                        @if ($item->practica->path_acta_revison)
                                        <br>
                                        Acta de revisión
                                        <a target="_blank" href="{{ route('asesor.comision.ver-acta-revision',['practica'=>$item->practica]) }}" class="font-medium text-blue-600 underline dark:text-blue-500 hover:no-underline">
                                            {{$item->practica->path_acta_revison}}
                                        </a>
                                        <br>
                                        @endif
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        @if (Auth::user()->asesor->comision()->latest()->first()->pivot->cargo == 'Presidente')
                                        <div class="inline-flex rounded-md shadow-sm" role="group">
                                            ...
                                            <button data-modal-target="subir-revision-modal-{{$item->id}}" data-modal-toggle="subir-revision-modal-{{$item->id}}" type="button" class="px-4 py-2 text-sm font-medium text-gray-900 bg-white border-t border-b border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-blue-500 dark:focus:text-white">
                                                Subir revisión
                                            </button>
                                            <a target="_blank" role="button" href="{{route('asesor.comision.generateActaRevisionView',['practicante'=>$item])}}" class="px-4 py-2 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-e-lg hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-blue-500 dark:focus:text-white">
                                                Revisión Prácticas
                                            </a>
                                        </div>
                                        @endif
                                    ...
                                    <div class="relative p-4 w-full max-w-md max-h-full">
                                        ...
                                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                            ...
                                            <div class="p-4 md:p-5">
                                                <form action="{{route('asesor.comision.updateFechaSustentacion',['id'=>$item->id])}}" method="post">
                                                    <p class="text-sm text-gray-900">Fecha y hora: {{date("d/m/Y", strtotime($item->practica->fecha_sustentacion))}} - {{date("H:i", strtotime($item->practica->fecha_sustentacion))}} </p>
                                                    <br>
                                                    @csrf
                                ...
                                <!-- Modal para la fecha -->
                                <div id="subir-revision-modal-{{$item->id}}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                    <div class="relative p-4 w-full max-w-md max-h-full">
                                        <!-- Modal content -->
                                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                            <!-- Modal header -->
                                            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                                                <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                                                    Subir acta de revisión
                                                </h3>
                                                <button type="button" class="end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="subir-revision-modal-{{$item->id}}">
                                                    <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                                                    </svg>
                                                    <span class="sr-only">Close modal</span>
                                            ...
                                            <!-- Modal body -->
                                            <div class="p-4 md:p-5">
                                                <form action="{{route('asesor.comision.subirActaRevision',['practicante' => $item])}}" method="post" enctype="multipart/form-data">
                                                    @csrf
                                                    <div class="grid gap-4 mb-4 grid-cols-2">
                                                        <div class="col-span-2">
                                                            <label for="path_acta_revison" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Acta de revisión firmada:</label>
                                                            <input accept=".pdf" type="file" name="path_acta_revison" id="path_acta_revison" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required="">
                                                        </div>
                                                    </div>
                                                    <button type="submit" class="text-white inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                                        Guardar
