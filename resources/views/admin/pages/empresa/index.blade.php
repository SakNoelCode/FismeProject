        ...
        <div id="modalVer" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
            <div class="relative p-4 w-full max-w-xl h-full md:h-auto">
                ...
                <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                    ...
                    <dl>
                        <dt class="mb-2 font-semibold leading-none text-gray-900 dark:text-white">Información:</span></dt>
                        <dd class="font-light text-gray-500 dark:text-gray-400"><span id="ver_description"></span></dd>
                        <dd class="font-light text-gray-500 dark:text-gray-400"><span id="ver_address"></span></dd>
                        <dd class="font-light text-gray-500 dark:text-gray-400"><span id="ver_city"></span></dd>
                        <dd class="font-light text-gray-500 dark:text-gray-400"><span id="ver_email"></span></dd>
                        <dd class="font-light text-gray-500 dark:text-gray-400"><span id="ver_phone"></span></dd>
                        <dd class="font-light text-gray-500 dark:text-gray-400"><span id="ver_web_url"></span></dd>
                        <input type="hidden" name="ver_empresa_id" id="ver_empresa_id">
                    </dl>
                    <div class="flex justify-between items-center">
                        <div class="flex items-center space-x-3 sm:space-x-4">

                        </div>
                        <button id="btnEliminarEmpresa" type="button" class="inline-flex items-center text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-red-500 dark:hover:bg-red-600 dark:focus:ring-red-900">
 ...
 */
    ...
    $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status == 404) {
                        ...
                        $('#ver_name').html(response.data.name);
                        $('#ver_phone').html(response.data.phone);
                        $('#ver_email').html(response.data.email);
                        $('#ver_address').html(response.data.address);
                        $('#ver_web_url').html(response.data.web_url);
                        $('#ver_city').html(response.data.city);
                        $('#ver_description').html(response.data.description);
                        $('#ver_empresa_id').val(response.data.id);
                        openModalVer();
                    }
                }
            });
        ...
        $(document).on('click', '#btnEliminarEmpresa', function(event) {
            event.preventDefault();
            $(this).text('Eliminando');
            let empresa_id = $('#ver_empresa_id').val();
