            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">Editar proyecto de tesis</span>
                    </div>
                </li>
            </ol>
        </nav>
            ...
            <div class="p-2 sm:p-4 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <header>
                    ...
                    <p class="my-1 text-sm text-gray-600 dark:text-gray-400">
                        {{ __("Por seguridad, solo podrá editar algunos campos") }}
                    </p>

                </header>
            </div>

            <h2 class="my-4 text-xl font-bold text-gray-900 dark:text-white">Editar proyecto de tesis</h2>

            <form action="{{route('proyectos.update',['proyecto'=>$proyecto])}}" method="post">
                @method('PATCH')
                @csrf
                ...
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="name" id="name" value="{{old('name',$proyecto->name)}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required="">
                    </div>
                    <div>
                        <label for="tesista_id" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tesista*:</label>
                        <select disabled id="tesista_id" name="tesista_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            <option value="{{$proyecto->tesista->id}}">{{$proyecto->tesista->user->name}}</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select id="asesor_id" name="asesor_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            ...
                            @if ($item->id == $proyecto->asesor_id)
                            <option selected value="{{$item->id}}" {{old('asesor_id') == $item->id ? 'selected' : ''}}>{{$item->user->name}}</option>
                            @else
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <select id="empresa_id" name="empresa_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            ...
                            @if ($item->id == $proyecto->empresa_id)
                            <option selected value="{{$item->id}}" {{old('empresa_id') == $item->id ? 'selected' : ''}}>{{$item->name}}</option>
                            @else
