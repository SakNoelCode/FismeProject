<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div id="fullWidthTabContent-{{$item->id}}" class="border-t border-gray-200">
                    ...
                    <div class="hidden p-4 bg-white rounded-lg" id="documentos-{{$item->id}}" role="tabpanel" aria-labelledby="documentos-tab-{{$item->id}}">
                        <ul>
                            ...
                            @if ($item->path_acta_sustentacion != null)
                            <li>
                                <a data-modal-target="resolucion-final-modal-{{$item->id}}" data-modal-toggle="resolucion-final-modal-{{$item->id}}" class="cursor-pointer inline-flex items-center font-medium text-blue-600 dark:text-blue-500 hover:underline mb-4 ml-4">
                                    Subir resolución de informe final
                                    <svg class="w-4 h-4 ms-2 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 10">
                                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 5h12m0 0L9 1m4 4L9 9" />
                                    </svg>
                                </a>
                            ...
                            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    ...
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                        ...
                                        <td class="px-6 py-4">
                                            @if ($acta->tipoacta_id == 1 || $acta->tipoacta_id == 7)
                                            Documento:
                                            ...
                                            @if ($acta->cargo_path)
                                            Cargo:
                                            <a target="_blank" href="{{ route('secretaria.practica.ver-pdf',['name'=>$acta->cargo_path]) }}" class="font-medium text-blue-600 underline dark:text-blue-500 hover:no-underline">
                                                {{$acta->cargo_path}}
                                            </a>
                                            @endif

                                            @if ($acta->pase_proveido)
                                            <br>
                                            <br>
                                            Proveído:
                                            <br>
                                        ...
                                        <td class="px-6 py-4">
                                            @if ($acta->tipoacta->id == 1 || $acta->tipoacta->id == 7)
                                            <button id="" data-dropdown-toggle="dropdown-{{$acta->id}}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
                                                Acciones
                                            ...
                                            <!-- Dropdown menu -->
                                            <div id="dropdown-{{$acta->id}}" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">
                                                <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                                                    <li>
                                                        <a role="button" data-modal-target="cargo-modal-{{$item->id}}" data-modal-toggle="cargo-modal-{{$item->id}}" class="cursor-pointer block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                                            Subir cargo
                                                        </a>
                                                    </li>
                                                    @if ($acta->pase_proveido == null)
                                                    <li>
                                                        <a role="button" data-modal-target="derivarExpedienteModal-{{$item->id}}" data-modal-toggle="derivarExpedienteModal-{{$item->id}}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                                            Realizar proveído
                                                        </a>
                                                    </li>
                                                    @endif
                                    ...
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            Acta de sustentación de prácticas
                                        </th>
                                        <td class="px-6 py-4">
                                            @php
                                            $rutaCompleta = $item->path_acta_sustentacion;
                                            $nombreArchivo = basename($rutaCompleta);
                                            @endphp
                                            <a target="_blank" href="{{ route('secretaria.practica.ver-pdf',['name'=>$nombreArchivo]) }}" class="font-medium text-blue-600 underline dark:text-blue-500 hover:no-underline">
                                                {{$nombreArchivo}}
            ...
            <!-- Modal para subir el cargo -->
            <div id="cargo-modal-{{$item->id}}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                <div class="relative p-4 w-full max-w-md max-h-full">
                    <!-- Modal content -->
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        <!-- Modal header -->
                        <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                            ...
                            <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="cargo-modal-{{$item->id}}">
                                <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                                </svg>
                                <span class="sr-only">Close modal</span>
                        ...
                        <!-- Modal body -->
                        <form action="{{route('secretaria.practicas.uploadCargo',['practica'=>$item->id,'id'=>$item->id])}}" class="p-4 md:p-5" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="grid gap-4 mb-4 grid-cols-2">
                                <div class="col-span-2">
                                    <label for="cargo_path" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Subir archivo: </label>
                                    <input accept=".pdf" required type="file" name="cargo_path" id="cargo_path" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5">
                                </div>
                            </div>
                            <button type="submit" class="text-white inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                Guardar
            ...
            <div id="derivarExpedienteModal-{{$item->id}}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                <div class="relative w-full max-w-md max-h-full">
                    ...
                    <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                        ...
                        <!-- Modal body -->
                        <form action="{{ route('secretaria.practicas.crearProveidoSolicitud',['practica'=>$item,'id'=>$item->id]) }}" method="post">
                            @csrf
                            @method('PATCH')
                            <div class="grid gap-4 mb-4 sm:grid-cols-2">
                                ...
                                <div class="col-span-2">
                                    ...
                                    <input type="text" name="pase" id="pase" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" required value="Comisión permanente">
                                </div>
            ...
            <!-- Modal para subir la resolución de informe final -->
            <div id="resolucion-final-modal-{{$item->id}}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                <div class="relative p-4 w-full max-w-md max-h-full">
                    <!-- Modal content -->
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        <!-- Modal header -->
                        <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                                Subir resolución de informe final de prácticas
                            </h3>
                            <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="resolucion-final-modal-{{$item->id}}">
                                <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                                </svg>
                                <span class="sr-only">Close modal</span>
                        ...
                        <!-- Modal body -->
                        <form action="{{route('secretaria.practicas.store-informe-final-practica',['practica'=>$item->id])}}" class="p-4 md:p-5" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="grid gap-4 mb-4 grid-cols-2">
                                <div class="col-span-2">
                                    <label for="documento_path" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Subir archivo: </label>
